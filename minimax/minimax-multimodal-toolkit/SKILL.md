...
Default model: `music-2.6`（推荐）

Available models:
- `music-2.6`：推荐，Token Plan/付费用户，RPM 较高
- `music-2.6-free`：`music-2.6` 限免版，所有用户可试，RPM 低
- `music-cover`：翻唱生成，需 Token Plan/付费用户

**`music-2.6` 支持 `is_instrumental: true` 参数直接生成纯音乐**，无需旧版的 `[intro] [outro]` workaround。使用 `--instrumental` 时 prompt 必填，lyrics 非必填。

**有歌词模式**：lyrics 必填，prompt 可选。

**自动歌词模式**：`lyrics_optimizer: true` + prompt（lyrics 为空），系统自动生成歌词。
