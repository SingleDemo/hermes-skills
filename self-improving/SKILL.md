...
## Pending/Heartbeat Automation (Advanced)

For recurring self-improving workflows, the following automation system is available:

- `~/self-improving/pending/` — inbox for quick task logging
- `~/self-improving/scripts/log-pending.sh <summary> [detail]` — record a task
- `~/self-improving/scripts/heartbeat.py` — process pending/, route to memory.md/corrections.md by keyword, update index + state
- Keyword routing:
  - `纠正|错误|修正|wrong|mistake|fix|bug` → corrections.md
  - `学会|learned|新技能|skill|workflow` → memory.md
  - other → memory.md
- Processed entries archive to `~/self-improving/archive/`
- Shell aliases: `log <summary>` / `hb` (heartbeat) / `si` (view dir)

**Shell gotcha**: When writing a shell script that passes variables to Python via heredoc, do NOT use `bash -c 'python3 - <<EOF ... $VAR ... EOF'` — bash variable expansion does NOT work inside single-quoted heredocs. Instead pass values via environment variables (`export VAR=value`) and read in Python with `os.environ[]`. Prefer pure Python for complex scripts that mix string interpolation and file I/O.
