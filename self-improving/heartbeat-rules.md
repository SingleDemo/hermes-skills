...
- `~/self-improving/pending/` is the inbox for tasks that need review
- Call `log-pending.sh <summary> [detail]` to record a task quickly
- Heartbeat processes entries in `pending/` automatically:
  - **correction** keywords (错误/纠正/wrong/mistake/fix/bug) → `corrections.md`
  - **learned** keywords (学会/learned/新技能/skill/workflow) → `memory.md`
  - other → `memory.md`
  - processed entries move to `~/self-improving/archive/`
- Run `python3 ~/self-improving/scripts/heartbeat.py` to process manually (pure Python, avoids bash heredoc variable expansion issues)
